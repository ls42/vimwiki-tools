...
def get_files(wiki_dir):
    for _, _, files in os.walk(wiki_dir, followlinks=True):
        return [f for f in files if os.path.splitext(f)[0] != "index"]
...
def get_links(wiki_dir):
    ...
    return [
        os.path.splitext(f)[0] for f in filenames if os.path.splitext(f)[0] != "index"
    ]
...
def output_as_html(links):
    ...
    def get_anchor(link, count):
        font_size = min_font_size + (max_font_size - min_font_size) / (
            max_reference - min_reference
        ) * (count - min_reference)
        return '<a href="%s.html" style="font-size:%dpx">%s</a>' % (
            link,
            round(font_size),
            link,
        )
